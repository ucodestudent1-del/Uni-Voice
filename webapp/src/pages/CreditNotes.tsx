...
export default function CreditNotes() {
  ...
  if (loading && creditNotes.length === 0) return <div className="text-center py-20 text-slate-500">Loading credit notes...</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">{total} credit notes total</p>
        </div>
        <button
        ...
        <table className="w-full">
          ...
          <tbody>
            {creditNotes.length === 0 ? (
              ...
              creditNotes.map((cn) => (
                <tr key={cn.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50">
                  <td className="py-3 px-4">
                    <div className="flex flex-col">
                      <Link to={`/app/credit-notes/${cn.id}`} className="text-sm font-medium text-slate-900 hover:text-primary-600">
                  ...
                  <td className="py-3 px-4 text-sm text-slate-600">
                    {cn.customer_name || "—"}
                    {cn.customer_email && <span className="text-xs text-slate-400 block">{cn.customer_email}</span>}
                  </td>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-900">Create Credit Note</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button type="button" onClick={addItem} className="text-xs text-primary-600 hover:text-primary-700">+ Add Item</button>
            </div>
